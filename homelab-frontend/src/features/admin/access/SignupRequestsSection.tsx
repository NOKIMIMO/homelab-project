...
import type { SignupRequest } from '../services/accessService';
import { STATUS_BADGE } from './statusBadge';

interface Props {
  pending: SignupRequest[];
  processed: SignupRequest[];
  actionId: number | null;
  onApprove: (id: number) => void;
  onReject: (id: number) => void;
}

export default function SignupRequestsSection({ pending, processed, actionId, onApprove, onReject }: Props) {
  return (
    <>
      {pending.length > 0 && (
        <section>
